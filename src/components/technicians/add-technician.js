...
class AddEditTechnician extends Component {
  ...
  handleSubmit(e) {
    ...
    if ((!id, !name || !password || !confirmPassword || !email || !phone)) {
      return this.setState({
        errors: {
          ...this.state.errors,
          id: !id,
    ...
    if (this.props.employees.find(employee => employee.id === parseInt(id, 10))) {
      return this.setState({
        errors: {
          id: true,
          message: 'Another employee with that ID already exists'
        }
      });
    }
    ...
    this.props.createEmployee({ id, name, email, phone, password, type, media_id }).then(res => {
        ...
        <form className="form">
          ...
          <div className="field">
            <div className="control">
              <input
                type="password"
                className={`input ${this.state.errors.confirmPassword ? 'is-danger' : ''}`}
                ref="confirmPassword"
              ...
              <div className="file is-white vd-box-shadow">
                <div htmlFor="file" className="file-label">
                  ...
                  >
                    ...
                    <span className="file-label has-text-grey-dark">Upload Profile Picture</span>
                  </span>
                  {this.renderFileName()}
                </div>
              </div>
