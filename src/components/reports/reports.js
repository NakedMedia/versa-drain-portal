...
class Reports extends Component {
  constructor(props) {
    ...
    this.state = {
      employee:
        props.employees && props.employees.find(employee => employee.id === userId) ? userId : null,
      client: props.clients && props.clients.find(client => client.id === userId) ? userId : null,
      search: ''
    };
  }
  ...
  renderReports(reports) {
    if (reports.length === 0) return <h3>No Reports</h3>;
      ...
      <div className="box" key={report.id}>
        <nav className="level is-mobile">
          ...
          <div className="level-left level-vertical is-hidden-mobile">
            <p className="has-text-grey-light is-size-7">{moment(report.date).format('lll')}</p>
          </div>
